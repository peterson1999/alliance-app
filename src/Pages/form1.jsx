...
class MyForm extends React.Component {
	...
	handleChange = (date) => {
		...
		this.state.data.datetime_issued = date;
	};
	...
	handleSubmit = () => {
		Axios.post('/v1/violations/', this.state).then((res) => console.log(res));
	};
		...
		return (
			<form>
				...
				<div>
					...
					<DateTimePicker value={this.state.startDate} onChange={this.handleChange} name="startDate" />
				</div>
				<div>
				...
				<div>
					<p>Date Due</p>
					<DateTimePicker value={this.state.dueDate} onChange={this.handleDeadlineChange} name="startDate" />
				</div>
				<div>
					<p>Status of Vehicle</p>
					<select type="text" name="status" onChange={this.myChangeHandler}>
						<option value="Towed">Ford</option>
						<option value="Clamped" selected>
							Volvo
						</option>
						<option value="On Hold">Fiat</option>
					</select>
				</div>
				<div>
